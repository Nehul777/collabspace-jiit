...
export function StudentCard({ student }: StudentCardProps) {
  ...
  const hardware = student.user_hardware || [];
  ...
  return (
    <div className="glass-card neon-glow-hover p-5 flex flex-col gap-4 transition-all duration-300 group h-full relative overflow-hidden">
      ...
      <div className="pt-4 mt-auto border-t border-white/5 flex items-center justify-between gap-2">
        <button className="flex-1 bg-accent/10 hover:bg-accent text-accent hover:text-white border border-accent/20 hover:border-accent rounded-lg py-2 text-xs font-medium transition-colors">
          Invite to Project
        </button>
        <Link href={`/profile/${student.id}`} className="px-3 py-2 rounded-lg bg-elevated hover:bg-white/10 border border-white/5 text-xs text-white/70 transition-colors">
          View
        </Link>
      </div>
    </div>
